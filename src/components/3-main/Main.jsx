...
export default function Main() {
  ...
  const buttons = ["All Projects", "HTML & CSS", "JavaScript", "React", ".Net MVC", ".Net API"];
  ...
  return (
    <main className='flex main-container' id='projects'>
      ...
      <section className='flex right-section'>
        {arr.map((item) => (
          <article key={item.projectTitl} className='card'>
            {/* التعديل هنا: إضافة / بين المسارين لضمان القراءة الصحيحة */}
            <img src={`${import.meta.env.BASE_URL}/${item.imgPath}`.replace('//', '/')} alt='' /> 
            ...
            <div style={{ width: "266px" }} className='box'>
              ...
              <div style={{ gap: "11px" }} className='flex icons'>
                ...
                <a className='link flex' href={item.live} target='_blank' rel="noreferrer">
                  More <BsArrowRight className='icon-arrow' />
                </a>
              </div>
            </div>
